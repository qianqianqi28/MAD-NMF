function [EMs, Abunds, Xprime] = MAD_NMF_SimplexProj(X, k, MaxIter, Reps, Regs, Verbose)
%
...
for Rep_ind = 1:Reps
    ...
    S0 = S0./repmat(sum(S0,2), 1,nVar);% Sum-to-ones
        ...
        for k_ind = 1:k
           ...
           % Update Sk, the end members
            ...
            %     / (norm(Ak).^2*eye(nVar) + b2.*(eye(nVar) - ones(nVar)./nVar) .* (1-1/k)^2) ;
             Sk = (Ak'*Xk - ( (b2/k)*(1-1/k)*S_sum' )' )...
                / (norm(Ak)^2 - b2*((1-1/k)^2)) ;
            ...
            % S(k_ind,:) = Sk;
            % QQ add
            Sk = SimplexColProj(Sk'); % Project rows of H onto the simplex
            Sk = Sk'; 
            S(k_ind,:) = Sk;
