...
% add noise
noise_factor = 1 + 0.01*randn(n_specimens, nBins);
% noise_factor = 1 + 0.1*randn(n_specimens, nBins);
noisy_samples = highly_mixed_samples .* noise_factor;

% Avoid negative values
noisy_samples(noisy_samples < 0) = 0;
% Renormalise to 100% per sample
noisy_coversand_samples = noisy_samples ./ sum(noisy_samples,2);


fprintf('Coversand dataset (200 samples, noise‑free):\n');
fprintf('mean abundances: %.3f %.3f %.3f %.3f\n', mean(abund_highmix));
fprintf('abundance ranges: [%.1f, %.1f]%%\n', ...
        min(abund_highmix(:)), max(abund_highmix(:)));
fprintf('sample sum check: sum = %f (should be 1)\n', mean(sum(noisy_coversand_samples,2)));


X = noisy_coversand_samples;
...
ylim([0 0.35])
exportgraphics(gcf,'plots/noisycoversanddataminems.pdf','ContentType','vector');
...
xlim([0 1])
ylim([0 1])
exportgraphics(gcf, 'plots/noisycoversanddataminabundances.pdf', 'ContentType', 'vector');
...
ylabel('Volume content', 'FontSize', 26, 'FontWeight', 'bold');
ylim([0 0.35])
exportgraphics(gcf,'plots/noisycoversanddatamaxems.pdf','ContentType','vector');
...
xlim([0 1])
ylim([0 1])
exportgraphics(gcf,'plots/noisycoversanddatamaxabundances.pdf','ContentType','vector');
